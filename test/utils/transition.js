...
export const getElementChildNodes = el => {
  ...
  return [...new Set(childNodes.reduce((acc, curr) => {
...
export const getElementAndChildrenTransitionProps = el => {
  ...
  const elementChildrenTransitionProps = elementChildren.map(getElementTransitionProps);
...
export const waitForTransitionEnd = async el => {
  ...
  const timeToTransition = elementTransitions.reduce((prev, curr) => {
    const transitionTotalDuration = parseTransitionTime(curr.transitionDuration) + parseTransitionTime(curr.transitionDelay);
